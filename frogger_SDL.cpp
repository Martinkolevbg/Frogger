...
// Date: 13/1/2020
// Authors: Martin Kolev , Kaloyan Glogovski & lyubo.shkoda.ta
...
// Global Variables
SDL_Rect windowRect = {900, 200, 500, 500};
...
void RunGame(){
    ...
    while(loop){
        ...
        if(playerPos.y < (topBar.y + topBar.h)){
            ...
            int count = 0;
            for (auto &p : enemies){
                p.speed = truckSpeeds[count] * 0.3;
                count++;
            }
            count = 0;
            for (auto &p : logs){
                p.speed = logSpeeds[count] * 0.3;
                count++;
            }
...
bool CreateWindow(){
    window = SDL_CreateWindow("Frogger", windowRect.x, windowRect.y, windowRect.w, windowRect.h, 0);
...
void AddEnemy(){
    ...
    if((rand() % 2 ) == 0){
        enemies.push_back(Enemy({rand() % 100, lastEnemyPos, 35, 20}, speed, Direction::Right));
        enemies.push_back(Enemy({rand() % 100 + 75, lastEnemyPos, 35, 20}, speed, Direction::Right));
        enemies.push_back(Enemy({rand() % 100 + 175, lastEnemyPos, 35, 20}, speed, Direction::Right));
    }
    else{
        enemies.push_back(Enemy({rand() % 100, lastEnemyPos, 35, 20}, speed, Direction::Left));
        enemies.push_back(Enemy({rand() % 100 + 75, lastEnemyPos, 35, 20}, speed, Direction::Left));
        enemies.push_back(Enemy({rand() % 100 + 175, lastEnemyPos, 35, 20}, speed, Direction::Left));
    }
...
void AddLog(Direction dir){
    int speed = rand() % 2 + 1; // rand spped for entire row
